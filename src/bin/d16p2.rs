...
fn process(input: String) -> usize {
    ...
    let origin_grid = Array2::from_shape_vec((nb_rows as usize, nb_cols as usize), input.into_iter().flatten().map(|c| (match c {
    ...
    for (start_y, start_x) in iproduct!(0..nb_rows, 0..nb_cols) {

        let mut grid = origin_grid.clone();

        let init_direction = match (start_y, start_x) {
            (y, x) if y == 0 => Direction::Down,
            (y, x) if y == nb_rows - 1 => Direction::Up,
            (y, x) if x == 0 => Direction::Right,
            (y, x) if x == nb_cols - 1 => Direction::Left,
            _ => continue,
        };
        
        let mut beams = vec![(init_direction.clone(), (start_y, start_x))];
        ...
        let count = grid.iter().filter(|(_, beamspassed)| !beamspassed.is_empty()).count();

        println!("x: {}, y: {}, dir: {:?}, found: {}", start_x, start_y, init_direction, count);



        if count > max.1 {
            max = ((start_x, start_y), count);
        }
    }
